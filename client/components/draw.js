...
const Draw = props => {
  const { drawCard, invalidDraw, deckSize, drawShadow, gameId } = props
  const goodDraw = !invalidDraw ? 'draw' : ''
  const shuffle = deckSize === 0 ? 'shuffle' : ''
  return (
    <div id='deck' className={`card card-container ${goodDraw} draw-border`} style={{ boxShadow: drawShadow }} onClick={() => drawCard(gameId)}>
